...
.. code-block:: shell
  ...
  vi package/gluon-nftables/luasrc/lib/gluon/upgrade/300-nftables
...
Packages don't install firewall rules directly, but ship them as Lua files in
``/lib/gluon/nftables``. All of these files are evaluated in lexical order by the
``300-nftables`` upgrade script of the *gluon-nftables* package. The script generates
``.nft`` files in the same directory and registers these, together with the ``.nft``
files shipped by the packages themselves, as ``include`` sections named
``gluon_nftables_*`` in the ``firewall`` UCI config, so they are applied by
OpenWrt's *fw4* on every firewall reload.
Includes from previous runs are removed before the rule files are evaluated.

The rule files are executed in an environment providing the following functions:

* *include(name, parameters)*: Registers the file ``/lib/gluon/nftables/<name>.nft``
  shipped by the package as an *fw4* include. The *parameters* are added to the
  generated UCI section; *position* and *chain* determine where *fw4* inserts the
  ruleset (for example ``position = 'chain-pre'`` and ``chain = 'dstnat'``).

* *rule(name, chain, content)*: Like *include()*, but writes *content* to the
  ``.nft`` file instead of using a file shipped by the package.

* *bridge_chain(name, policy, table)*, *bridge_rule(chain, content, table)*,
  *bridge_table(position, content)*: Define chains and rules of the ``bridge gluon``
  table, which contains the layer 2 filtering applied between the client bridge and
  the mesh. This table is generated as a single ``bridge.nft`` file which is included
  before the rest of the ruleset.

  *table* selects one of the two sets of base chains, which correspond to the
  ``filter`` and ``nat`` tables of ebtables and only differ in the netfilter
  priority they are registered at. It defaults to ``filter``, which provides the
  base chains *INPUT*, *FORWARD* and *OUTPUT*; ``nat`` runs earlier and provides
  *PREROUTING*, *OUTPUT* and *POSTROUTING*. Chains created by *bridge_chain()* are
  regular nft chains, for which *table* only determines which base chains may jump
  into them.

  *policy* is the verdict applied to packets reaching the end of a chain created by
  *bridge_chain()*; it defaults to ``RETURN`` and may also be ``DROP`` or ``ACCEPT``,
  which are emitted as a terminal rule after all rules of the chain.

* *bridge_include_rule(chain, file, table)*, *bridge_include_table(position, file)*:
  Insert the contents of a ``.nft`` file shipped by the package into the
  ``bridge gluon`` table, for rulesets that are too complex to be written inline.

As the rules are generated by an upgrade script, changes to the rule files take
effect after running ``gluon-reconfigure`` and ``gluon-reload``.
